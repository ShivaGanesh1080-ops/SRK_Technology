...
export default function DemoCertificatePage() {
  const studentName = "YOUR FULL NAME";
  const dateIssued = "Will be issued upon completion";
  const certificateId = "SRK-DEMO-XXXXX";
  const instructorName = "SRK TECHNOLOGY";
  const workshopTitle = "YOUR WORKSHOP TITLE";
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-12 px-4 print:bg-white print:p-0 print:py-0 flex flex-col items-center">
      <div className="w-full max-w-5xl mx-auto mb-8 print:hidden flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-amber-200">
        ...
        <Link href="/student" className="bg-slate-900 text-white px-6 py-2 rounded-md font-medium hover:bg-slate-800 transition-colors shadow-sm">
          Return to Dashboard
        </Link>
      </div>

      {/* Redesigned Certificate Layout (Watermarked) */}
      <div className="w-full max-w-5xl aspect-[1.414/1] bg-white p-4 md:p-8 shadow-2xl relative overflow-hidden print:shadow-none mx-auto opacity-90" style={{ backgroundImage: "url('https://www.transparenttextures.com/patterns/cubes.png')", border: "12px solid #1e293b" }}>
        {/* Watermark */}
        <div className="absolute inset-0 z-0 flex items-center justify-center rotate-[-30deg] pointer-events-none opacity-10">
           <span className="text-9xl font-black text-slate-900 tracking-widest">DEMO ONLY</span>
        </div>

        {/* Inner Border */}
        <div className="w-full h-full border-[6px] border-double border-slate-300 relative z-10 bg-white/90 p-12 flex flex-col items-center justify-between text-center">
          ...
          <div className="pt-8 w-full">
            <div className="flex items-center justify-center mb-6">
               <Award className="w-16 h-16 text-slate-400 mr-4" />
          ...
          <div className="flex-1 flex flex-col justify-center w-full max-w-3xl">
            ...
            <h2 className="text-5xl font-bold text-slate-400 mb-8 pb-4 border-b-2 border-dashed border-slate-300" style={{ fontFamily: "'Brush Script MT', cursive", fontSize: "1.5rem", color: "#1e3a8a" }}>
              {studentName}
            </h2>
            <p className="text-lg text-slate-600 italic font-serif mb-6">will successfully complete the training program in</p>
            <h3 className="text-3xl font-bold text-blue-800/50 uppercase px-8">
              {workshopTitle}
            </h3>
          </div>

          {/* Footer Signatures */}
          <div className="w-full max-w-4xl grid grid-cols-3 gap-8 mt-12 pt-8 pb-8 items-end opacity-50">
            ...
            <div className="flex flex-col items-center">
              <div className="w-32 border-b-2 border-slate-400 mb-2 pb-2">
                 <span className="text-sm text-slate-700 font-bold text-center leading-tight">Will be<br/>issued</span>
              </div>
